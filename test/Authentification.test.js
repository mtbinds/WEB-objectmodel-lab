...
describe("login", () => {
  ...
  it("Logs in and returns a token", done => {
    request(app)
      .post("/api")
      .send({
        query: '{ login(username: "max", password: "max") { token } }'
      })
      .expect(200)
      .end((err, res) => {
...
describe("signup", () => {
  ...
  it("Should return a 'user already exists' message", done => {
    request(app)
      ...
      .send({
        mutaion:
          '{ createUser(userInput: {username:"max", firstname: " max " ,lastname: " braver ", email: "maxybraver@gmail.com", password: "max", role: "admin"}) }'
      })
      .expect(200)
      .end((err, res) => {
